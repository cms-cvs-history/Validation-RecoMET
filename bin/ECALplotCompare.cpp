...
int main(int argc, char *argv[]) {
  ...
  PlotCompareUtility *pc = new PlotCompareUtility(argv[1],argv[2],"DQMData/METTask/ECAL/data","METTask_ECAL_");
  HistoData *hd;

  if (pc->GetStatus() != 0) { cout << "Final Result: no_data" << endl; return 0; }

  // add histogram information
  hd = pc->AddHistoData("EB_energyvsieta"); hd->SetType(2); hd->SetValueX("ECAL energy (GeV)");
  hd = pc->AddHistoData("EB_Occvsieta"); hd->SetType(2); hd->SetValueX("ECAL Occupancy");
  ...
  for (int index = 0; index < pc->GetNumHistos(); index++) {
    ...
    TH1F h1dResults_passed(titleP.c_str(),titleP.c_str(),171,-85,86);
    TH1F h1dResults_failed(titleF.c_str(),titleF.c_str(),171,-85,86);
	...
	*/
      ...
      if (fabs(XaxisMin_ref - XaxisMax_ref)<1E-6)
	{
      ...
      TString hdumb_name ="hdumb_"; hdumb_name += bin;
...
/*  }  else if (type == 1) {
    ...
    TLine l(-85, threshold, 86, threshold);
    l.SetLineColor(kRed);
    l.SetLineWidth(2);
    l.SetLineStyle(2);
    l.Draw("SAME");
